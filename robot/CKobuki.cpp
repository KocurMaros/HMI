...
std::vector<unsigned char> CKobuki::setTranslationSpeed(int mmpersec)
{
	unsigned char message[14] = {
		0xaa, 0x55, 0x0A, 0x0c, 0x02, 0xf0, 0x00, 0x01, 0x04, static_cast<unsigned char>(mmpersec % 256), static_cast<unsigned char>(mmpersec >> 8),
		0x00, 0x00, 0x00
	};
...
std::vector<unsigned char> CKobuki::setRotationSpeed(double radpersec)
{
	...
	unsigned char message[14] = {
		0xaa, 0x55, 0x0A, 0x0c, 0x02, 0xf0, 0x00, 0x01, 0x04, static_cast<unsigned char>(speedvalue % 256), static_cast<unsigned char>(speedvalue >> 8),
		0x01, 0x00, 0x00
	};
	message[13] = message[2] ^ message[3] ^ message[4] ^ message[5] ^ message[6] ^ message[7] ^ message[8] ^ message[9] ^ message[10] ^ message[11] ^ message[12];
...
std::vector<unsigned char> CKobuki::setArcSpeed(int mmpersec, int radius)
{
	...
	unsigned char message[14] = { 0xaa,
								  ...
								  0x01,
								  0x04,
								  static_cast<unsigned char>(speedvalue % 256),
								  static_cast<unsigned char>(speedvalue >> 8),
								  static_cast<unsigned char>(radius % 256),
								  static_cast<unsigned char>(radius >> 8),
								  0x00 };
	message[13] = message[2] ^ message[3] ^ message[4] ^ message[5] ^ message[6] ^ message[7] ^ message[8] ^ message[9] ^ message[10] ^ message[11] ^ message[12];
	uint32_t pocet;
	std::vector<unsigned char> vystup(message, message + sizeof(message) / sizeof(message[0]));
...
std::vector<unsigned char> CKobuki::setSound(int noteinHz, int duration)
{
	...
	unsigned char message[13] = { 0xaa,
								  ...
								  0x03,
								  0x03,
								  static_cast<unsigned char>(notevalue % 256),
								  static_cast<unsigned char>(notevalue >> 8),
								  static_cast<unsigned char>(duration % 256),
								  0x00 };
...
{
	unsigned char message[23] = { 0xaa, 0x55, 0x13, 0x0c, 0x02, 0xf0, 0x00, 0x0D, 0x0D, 0x00, 0x00, 0x00,
	...
	while (checkedValue < data[0]) {
		...
		if (data[checkedValue] == 0x01) {
			...
			if (data[checkedValue] != 0x0C)
				return -11;
			checkedValue++;
			output.extraInfo.UDID0 = data[checkedValue + 3] * 256 * 256 * 256 + data[checkedValue + 2] * 256 * 256 + data[checkedValue + 1] * 256
				+ data[checkedValue];
			checkedValue += 4;
			output.extraInfo.UDID1 = data[checkedValue + 3] * 256 * 256 * 256 + data[checkedValue + 2] * 256 * 256 + data[checkedValue + 1] * 256
				+ data[checkedValue];
			checkedValue += 4;
			output.extraInfo.UDID2 = data[checkedValue + 3] * 256 * 256 * 256 + data[checkedValue + 2] * 256 * 256 + data[checkedValue + 1] * 256
				+ data[checkedValue];
			checkedValue += 4;
		}
		else {
